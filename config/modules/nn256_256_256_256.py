...
weight_file_name = 'nn256_256_256_256_weights.pkl'
...
def create_weights(number_input, number_output, logger):
    ...
    if os.path.isfile(weight_file_name):
        ...
        weights = {
            ...
            'w_4': base.weight_variable([256,256]),
            'b_4': base.bias_variable([256]),

            'w_5': base.weight_variable([256,number_output]),
            'b_5': base.bias_variable([number_output])
        }
    return weights
...
def create_network(x, weights, keep_prob):
    ...
    h_4 = tf.nn.relu(tf.matmul(h_2, weights['w_4']) + weights['b_4'])
    h_drop = tf.nn.dropout(h_4, keep_prob)
    return tf.nn.softmax(tf.matmul(h_drop, weights['w_5']) + weights['b_5'])
